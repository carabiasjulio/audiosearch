...
\title{Interactive Audio Search}

% Note that submissions are blind, so author information should be omitted
\numberofauthors{1}
\author{
  \alignauthor Minshu Zhan\\
    \affaddr{Massachusetts Institute of Technology, EECS}\\
    \affaddr{Address}\\
    \email{mzhan@mit.edu}\\
}
...
Content-based query is hard. Need to search for sound without literal descriptions - and it's hard to describe in words anyways - e.g in long recording/videos, or invididual samples in large 
...
Sound database increase exponentially, just like any other form of media data. Effectively retrieving media content has been a problem. Personification especially difficult. 
In this paper we introduce an interactive audio retrieval system, SoundSearch, that search for audio sample database based on audio content and can  taylored to individual needs. through adaptive labelling and 
We evaluate the system in an simulated interactive labelling scheme and show that the interactive model improves the query accuracy by \%300, without  
